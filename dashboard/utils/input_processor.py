...
def cycle_to_number(value):
    """
    Convert cycle pattern dashboard input into numeric training format.
    """
...
def prepare_input(user_inputs):
    ...
    input_data = {
        ...
        'Cycle(R/I)': cycle_to_number(user_inputs.get('cycle_pattern', 2)),
        'Cycle length(days)': user_inputs.get('cycle_length', 28),

        'Marraige Status (Yrs)': user_inputs.get('marriage_status', 0),
        'Pregnant(Y/N)': yes_no_to_number(user_inputs.get('pregnant', 0)),
        ...
        'Weight gain(Y/N)': yes_no_to_number(user_inputs.get('weight_gain', 0)),
        'hair growth(Y/N)': yes_no_to_number(user_inputs.get('hair_growth', 0)),
        'Skin darkening (Y/N)': yes_no_to_number(user_inputs.get('skin_darkening', 0)),
        'Hair loss(Y/N)': yes_no_to_number(user_inputs.get('hair_loss', 0)),
        'Pimples(Y/N)': yes_no_to_number(user_inputs.get('acne', 0)),
        'Fast food (Y/N)': yes_no_to_number(user_inputs.get('fast_food', 0)),
        'Reg.Exercise(Y/N)': yes_no_to_number(user_inputs.get('regular_exercise', 0)),

        'BP _Systolic (mmHg)': user_inputs.get('bp_systolic', 120),
        'BP _Diastolic (mmHg)': user_inputs.get('bp_diastolic', 80),

        'Follicle No. (L)': user_inputs.get(
            'follicle_count_left',
            user_inputs.get('follicle_count', 5)
        ),
        'Follicle No. (R)': user_inputs.get(
    ...
    # Make sure every column is numeric before scaling
